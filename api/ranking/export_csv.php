...
use App\Core\{Middleware, Request, Bootstrap};

// Note: CORS headers handled differently for CSV export
...
$classId = Request::getInt('class_id') ?: 0;
...
if ($type === 'week') {
    if (!$classId || !$label) {
        ...
        echo "Missing class_id or label";
        exit;
    }
    $parts = explode('/', $label);
    if (count($parts) !== 2) {
    ...
    if (!$classId || !$year || !$semester) {
        http_response_code(400);
        echo "Missing class_id/year/semester";
        exit;
    }
